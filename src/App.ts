  ...
  constructor($target: HTMLElement) {
    ...
    const search = new Search({
      ...
      onRandomSearch: async () => {
        loader.show();
        search.$elInput.setState("");
        const randowData = await api.getRandom(1);
    ...
    const recentKeyword = new RecentKeyword({
      $target,
      callback: async (key) => {
        loader.show();
    ...
    const list = new List({
      ...
      onScroll: async (page: number) => {
        loader.show();
        const randowData = await api.getRandom(page);
        this.cats = randowData;
        list.setState(randowData);
        loader.hide();
      },
